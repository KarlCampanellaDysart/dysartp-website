...
setTimeout(function(){var k=j.get_odataClientSettings().ODataSourceID,m=$find(k);
if(!m){var l=String.format("DataSource with id {0} was not found on the page",k);
alert(l);
return;
}j._hierarhicalBinder=new a.RadODataDataSource.Binder.Hierarhical(m,j);
j._hierarhicalBinder.initialize();
var n=f.raiseCancellableControlEvent(j,"menuPopulating",{});
if(!n){j._onDataNeeded();
}},1);
},_onDataNeeded:function(m){var k=m!=undefined?1+m.get_level():0,j=m!=undefined?m.get_key():"null";
var l={level:k,key:j,events:{requesting:function(o){if(m){var n=new Telerik.Web.UI.WebServiceLoaderEventArgs(m);
this._onItemLoadingStarted(this,n);
}},success:function(p){var n=m||this,o=new Telerik.Web.UI.WebServiceLoaderSuccessEventArgs(p,n);
this._onItemLoadingSuccess(this,o);
},fail:function(n){}}};
...
j.ExpandMode=a.MenuItemExpandMode.WebService;
var k=this.get_dataBindings();
if(k){k[k.length-1].ExpandMode=a.MenuItemExpandMode.ClientSide;
}j.DataBindings=k;
return a.RadODataDataSource.Binder.Hierarhical.Model(j);
...
if(typeof k.get_level=="undefined"){break;
...
}}else{if(this.get_odataClientSettings()){this._onDataNeeded(m);
return;
...
k._renderedClientTemplate=a.TemplateRenderer.renderTemplate(m,this,k);
...
f.registerControlProperties(a.RadMenu,{clicked:false,dataBindings:null,odataClientSettings:null,enableRootItemScroll:false,enableRoundedCorners:false,enableShadows:false,enableImageSprites:false,clientTemplate:null,loadingTemplate:"",enableScreenBoundaryDetection:true,enableAutoScroll:false,enableSelection:true,autoScrollMinimumHeight:50,autoScrollMinimumWidth:50,clickToOpen:false,collapseDelay:500,expandDelay:10});
f.registerControlEvents(a.RadMenu,["load","menuPopulating","menuPopulated","itemFocus","itemBlur","itemOpening","itemOpened","itemClosing","itemClosed","itemPopulating","itemPopulated","itemPopulationFailed","templateDataBound","mouseOver","mouseOut","itemClicked","itemClicking"]);
...
}this._renderedClientTemplate=b.TemplateRenderer.renderTemplate(x,this.get_menu(),this);
...
j("a.rmLink",this._element).replaceWith(x);
j(this._element).addClass("rmTemplate");
...
if(this.get_groupSettings().get_flow()==b.ItemFlow.Vertical){return(x.get_autoScrollMinimumHeight()<y&&y<=z.offsetHeight);
}else{return(x.get_autoScrollMinimumWidth()<y&&y<=z.offsetWidth);
...
this._slide.updateSize();
var x=j(this.get_element()).hasClass("rmTemplate");
