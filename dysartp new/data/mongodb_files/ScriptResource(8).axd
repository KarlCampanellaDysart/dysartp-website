...
}this._renderedClientTemplate=a.TemplateRenderer.renderTemplate(c,this.get_treeView(),this);
...
}var f=this._element.getElementsByClassName("rtIn")[0],e=f.parentNode,c=document.createElement("div"),d=document.createElement("div");
c.className="rtIn";
d.className="rtTemplate";
d.innerHTML=this._renderedClientTemplate;
c.appendChild(d);
e.replaceChild(c,f);
...
}c.text=a.RadTreeView._htmlEncode(this.get_text());
...
if(this.get_navigateUrl()){var e=d.getAttribute("href");
d.removeAttribute("href");
d.setAttribute("hrefValue",e);
}this._originalTextHtml=d.innerHTML;
d.innerHTML="";
var g=document.createElement("input");
g.setAttribute("type","text");
g.setAttribute("size",this._originalText.length+3);
g.setAttribute("value",a.RadTreeView._htmlDecode(this._originalText));
this._inputElement=g;
this._addClassToContentElement("rtEdit");
d.appendChild(g);
var f=this;
g.onblur=function(){f._endEdit(false);
};
g.onchange=function(){f._endEdit(false);
};
g.focus();
this._cancelInputEvents(g);
this._selectInputText(g,this._originalText.length);
this.get_treeView()._raiseEvent("nodeEditStart",this);
},_endEdit:function(d){if(this._editing){this._editing=false;
var g=this.get_inputElement();
if(!g){return;
}var c=g.parentNode;
c.removeChild(g);
if(!d){this._updateText(c,this._originalText,this._originalTextHtml,g.value);
var e=this._originalText!=g.value;
if(!this.get_treeView()._editNodeText(this,g.value,e)){c.innerHTML=this._originalTextHtml;
}}else{c.innerHTML=this._originalTextHtml;
}if(this.get_navigateUrl()){var f=c.getAttribute("hrefValue");
c.removeAttribute("hrefValue");
...
},set_checked:function(c,f){if(!this.get_isEnabled()){return;
}var d=this.get_treeView();
this._check(d,c,f);
...
},clone:function(h){var f=["text","value","key","category","enabled","expanded","checked","navigateUrl","toolTip","imageUrl","disabledImageUrl","expandedImageUrl","selectedImageUrl","hoveredImageUrl","cssClass","disabledCssClass","hoveredCssClass","enableContextMenu","postBack","expandMode","allowEdit","allowDrag","allowDrop"];
var j=this;
...
e[e.length]="</span></div>";
}}},_renderClientTemplate:function(c,d){if(d){c[c.length]="<div class='rtIn ";
c[c.length]=d;
c[c.length]="'>";
}else{c[c.length]="<div class='rtIn'>";
}c[c.length]="<div class='rtTemplate'>";
...
}else{if(this._renderedClientTemplate){this._renderClientTemplate(f,j);
}else{if(j){f[f.length]="<span class='rtIn ";
f[f.length]=j;
f[f.length]="'>";
}else{f[f.length]="<span class='rtIn'>";
}if(c._enableNodeTextHtmlEncoding){f[f.length]=a.RadTreeView._htmlEncode(this._properties.getValue("text",""));
}else{f[f.length]=this._properties.getValue("text","");
}f[f.length]="</span></div>";
}}f[f.length]="</li>";
...
c++){if(this.get_parent().get_nodes().getNode(c).get_visible()){e++;
}}if(e<2){d="rtLI rtFirst rtLast";
...
setTimeout(function(){var i=h.get_odataClientSettings().ODataSourceID,k=$find(i);
if(!k){var j=String.format("DataSource with id {0} was not found on the page",i);
alert(j);
return;
}h._hierarhicalBinder=new a.RadODataDataSource.Binder.Hierarhical(k,h);
h._hierarhicalBinder.initialize();
var l=e.raiseCancellableControlEvent(h,"treePopulating",{});
if(!l){h._onDataNeeded();
}},1);
},_onDataNeeded:function(k){var i=k!=undefined?1+k.get_level():0,h=k!=undefined?k.get_key():"null";
var j={level:i,key:h,events:{requesting:function(m){if(k){var l=new Telerik.Web.UI.WebServiceLoaderEventArgs(k);
this._onNodeLoadingStarted(this,l);
}},success:function(n){var l=k||this,m=new Telerik.Web.UI.WebServiceLoaderSuccessEventArgs(n,l);
this._onNodeLoadingSuccess(this,m);
},fail:function(l){}}};
this._hierarhicalBinder.fetch(j);
},get_hierarchyModel:function(){var h=this.get_odataClientSettings();
h.ExpandMode=a.TreeNodeExpandMode.WebService;
var i=this.get_dataBindings();
if(i){i[i.length-1].ExpandMode=a.TreeNodeExpandMode.ClientSide;
}h.DataBindings=i;
return a.RadODataDataSource.Binder.Hierarhical.Model(h);
...
h++){var j=k[h];
if(j){j.trackChanges();
}}}a.RadTreeView.callBaseMethod(this,"trackChanges");
...
h=this._extractNodeFromDomElement(i.eventMapTarget);
if(!h){return;
...
if(h.ODataSettings){this._webServiceSettings=new a.NavigationControlODataSettings(h);
...
},_doLoadOnDemand:function(m){var l=new a.RadTreeNodePopulatingEventArgs(m,null);
this.raiseEvent("nodePopulating",l);
if(l.get_cancel()){m._properties.setValue("expanded",false);
return;
}var j=String.format('{{commandName:"LOD",index:"{0}",data:{1},clientState:{2}}}',m._getHierarchicalIndex(),c.serialize(m._getData()),this.saveClientState());
if(this.get_loadingStatusPosition()!=a.TreeViewLoadingStatusPosition.None){m.showLoadingStatus(this.get_loadingMessage(),this.get_loadingStatusPosition());
}var k=Function.createDelegate(this,this._onCallbackResponse);
var i=Function.createDelegate(this,this._onCallbackError);
var n=new a.CallbackSettings({id:this._uniqueId,arguments:j,onCallbackSuccess:k,context:m,onCallbackError:i,isAsync:true});
var h=new a.CallbackLoader(n);
...
if(h.get_isOData()){this._webServiceLoader=new a.NavigationControlODataLoader(this.get_webServiceSettings(),function(i){if(i.length>0){return a.TreeNodeExpandMode.WebService;
...
s._renderedClientTemplate=a.TemplateRenderer.renderTemplate(t[k],this,s);
Array.add(p,s);
}this.addNodesTo(n,p);
...
k._renderedClientTemplate=a.TemplateRenderer.renderTemplate(w[s],this,k);
Array.add(q,k);
}this.addNodesTo(j,q);
...
a.RadTreeView._htmlDecode=function(j){if(e.type(j)==="string"){var i={"&lt;":"<","&gt;":">","&amp;":"&"};
for(var h in i){j=j.replace(new RegExp(h,"g"),i[h]);
}return j;
}};
a.RadTreeView._htmlEncode=function(j){if(e.type(j)==="string"){var i={"&":"&amp;","<":"&lt;",">":"&gt;"};
for(var h in i){j=j.replace(new RegExp(h,"g"),i[h]);
}return j;
}};
